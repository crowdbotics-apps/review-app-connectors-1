...
function* fakeconnectoredit_get_pets_listWorker(action) {
  try {
    const result = yield call(
      apiService.fakeconnectoredit_get_pets_list,
      action
    )
    yield put(actions.fakeconnectoredit_get_pets_listSucceeded(result, action))
  } catch (err) {
    yield put(actions.fakeconnectoredit_get_pets_listFailed(err, action))
  }
}
function* fakeconnectoredit_get_pets_listWatcher() {
  yield takeEvery(
    types.FAKECONNECTOREDIT_GET_PETS_LIST,
    fakeconnectoredit_get_pets_listWorker
  )
}
...
export default function* rootSaga() {
  const sagas = [
    fakeconnectoredit_get_pets_listWatcher,
